...
function formatStringLiteralUnionType(
  typeName: string,
  values: string[],
): string {
  if (values.length === 0) {
  ...
  return [
    ...
    `${values.map((value) => `  | ${formatStringLiteral(value)}`).join('\n')};`,
  ].join('\n');
}
...
const generatedUnionsPattern =
  /\/\/ <AUTO-GENERATED:UNIONS>[\s\S]*?\/\/ <\/AUTO-GENERATED:UNIONS>/;
const unionBlock = [
  '// <AUTO-GENERATED:UNIONS>',
  formatStringLiteralUnionType('IconName', iconNames),
  formatStringLiteralUnionType('CategoryId', categoryIds),
  '// </AUTO-GENERATED:UNIONS>',
].join('\n');

if (!generatedUnionsPattern.test(typesSource)) {
  throw new Error(`Missing generated unions block in ${typesPath}`);
}
